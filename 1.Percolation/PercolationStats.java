...
public class PercolationStats
{
	...
	{
		//mu = (x1 + x2 .. xn) / _times
		return StdStats.mean(_openSites);
	}

	public double stddev()
	{
		// stdev[rho^2] = [(x1 - mu)^2 + (x2 - mu)^2 + .. (xn - mu)^2] / (_times - 1)
		return StdStats.stddev(_openSites);
	}

	public double confidenceLo()
	{
		//95% Confidence interval: { mu ± [(1.96 * rho) / sqrt(T)] }
		return (mean() - ((1.96 * stddev())/Math.sqrt(_openSites.length)));
	}
